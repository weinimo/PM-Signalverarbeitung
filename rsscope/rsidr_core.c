    ...
    switch (sessionProperties -> attr[idx].dataType)
        ...
        case RS_VAL_BOOLEAN:
            ...
            if ((sessionProperties -> attr[idx].flags & RS_VAL_COERCEABLE_ONLY_BY_INSTR) == 0)
                ...
                strncpy (coercedValue, magic_code, strlen (magic_code)); /* Mark buffer with the magic code  */
...
Update_Cache: /* Update value in the cache */
    ...
    if (coercedValue != NULL)
        if (strncmp (coercedValue, magic_code, strlen (magic_code)) != 0)
        {
                       ...
                       ViString cmdValue)
{
    ...
    if (!rep_cap_exists)
    {
        ...
        p2buf = realloc (sessionProperties->repCapTable[i].repCapName, new_size);
        ...
        p2buf = realloc (sessionProperties->repCapTable[i].cmdValues, new_size);
                          ...
                          ViString repCapName)
{
    ...
    new_buffer = (ViChar *) malloc (new_size);
                               ...
                               ViChar cmdValue[])
{
    ...
    //    if ((Rs_GetIndexOfToken (repCapNameList, ",", &cmd_idx, repCapName) != VI_SUCCESS))
        {
...
ViStatus Rs_ApplyAttributeDefaults (ViSession io, ViBoolean updateInherentAttr)
{
    ...
    for (idx = 0; idx < sessionProperties -> numberOfAttr; idx++)
        ...
        if ((updateInherentAttr == VI_FALSE) && 
            ...
            memset (p2value, 0, data_size);
            memcpy (p2value, p2default, data_size);
            }
        }
 ...
 *****************************************************************************/
ViStatus Rs_GetTokenAtIndex (ViChar *string_to_separate,
                             ViConstString delimiter,
                             ViUInt32 index,
                             ViChar token[],
                             ViUInt32 token_max_length)
{
    ViStatus    error   = VI_SUCCESS;
    ViUInt32    cnt     = 0;
    size_t      n       = 0;
    ...
    if ((token == NULL) || (string_to_separate == NULL))
    ...
    while (p2string != NULL)
        ...
        if (index == cnt)
            {
            n = (token_max_length < token_length) ? (size_t) (token_max_length - 1) : token_length;
...
ViStatus Rs_CheckModelOpt (ViSession instrSession, ViAttr attributeId)
{
    ...
    ViInt32     idx             = 0;
    ViBoolean   boolean         = VI_FALSE;
    ViChar      options_list[RS_MAX_MESSAGE_BUF_SIZE] = "",
                options_string[RS_MAX_MESSAGE_BUF_SIZE] = "",
                instrument_model[RS_MAX_MESSAGE_BUF_SIZE] = "",
                token[RS_MAX_MESSAGE_BUF_SIZE] = "",
                tmp_instr_model[RS_MAX_MESSAGE_BUF_SIZE] = "";
                
    ViChar      *p2string       = VI_NULL,
                *p2token        = VI_NULL,
                operation;
    size_t      len = 0;
    // string_size = 0;  
    ...
    strncpy (options_list, (ViString)sessionProperties -> attr[idx].p2value, RS_MAX_MESSAGE_BUF_SIZE - 1);

    /* Get instrument model */
    viCheckErr (Rs_GetAttributeIndex (instrSession, RS_ATTR_INSTRUMENT_MODEL, &idx));
    strncpy (instrument_model, (ViString)sessionProperties -> attr[idx].p2value, RS_MAX_MESSAGE_BUF_SIZE - 1);

    /* Instrument model */
    viCheckErr (Rs_GetAttributeIndex (instrSession, attributeId, &idx));

    /* No model(s) listed, skip model checking */
    //strncpy(tmp_supported_model,sessionProperties -> attr[idx].supportedModels, strlen(sessionProperties -> attr[idx].supportedModels)); 
    if (sessionProperties -> attr[idx].supportedModels)
    {
        p2token=sessionProperties -> attr[idx].supportedModels;
        while (p2token){
            sscanf (p2token, "%[^|]", tmp_instr_model); 
...
/*        if (p2token)
         {
            while (p2token){
                len=strlen(p2token);
                if (strncmp(p2token,instrument_model,len))
                    break;
                p2token=strtok(NULL,"|&");
                p2token=strtok(NULL,"(");    
            } 
            //Model not found, try tmp_instr_model FSQ instead of FSQ-8
            //Not necessary, instrument model has to be only FSQ, FSU ...
            if (!p2token)
            {
                p2token=strtok(sessionProperties -> attr[idx].instrumentOptions, "(");  
                while (p2token){
                    len=strlen(p2token);
                    if (strncmp(p2token,tmp_instr_model,len))
                        break;
                    p2token=strtok(NULL,"|&");
                    p2token=strtok(NULL,"(");    
                }      
         ...
         if (p2token)
         {
            p2token = strchr (p2token,'(');
            if (p2token)
                sscanf (p2token,"(%[^)]", options_string);
          //  strncpy (options_string, sessionProperties -> attr[idx].instrumentOptions, string_size);
          //  options_string[RS_MAX_MESSAGE_BUF_SIZE-1] = 0;   
         }
         /*
         if (p2string = strstr (sessionProperties -> attr[idx].instrumentOptions, instrument_model))
         {
                p2string = strpbrk (p2string, "(");

                if (!p2string)
                    return RS_ERROR_INSTRUMENT_OPTION; // Wrong syntax 

                p2string++; // move after the '(' character 

                string_size = strpbrk (p2string, ")") - p2string;

                if (string_size < 0)
                    string_size = 0;
        
                strncpy (options_string, p2string, string_size);
                options_string[string_size] = 0;
         }
         else
         {
                strncpy (options_string, sessionProperties -> attr[idx].instrumentOptions, string_size);
        ...
        else
        {
            ...
            if (sessionProperties -> attr[idx].instrumentOptions)
            {*/
        else
        {
                                ...
                                ViChar **cmd)
{
    ...
    ViChar      repCapNames[RS_MAX_MESSAGE_BUF_SIZE*3] = "";
    size_t      max_cmd_size    = 0;
    ViInt32     n;
        ...
        /* Repeated Capabilities */
            ...
            for (index = 0; index < RS_MAX_ATTR_RCAPS; index++)  
            {
                ...
                for (i = 0; i < strlen (repCapNames); i++) 
                {
                    ...
                    /*if (strstr (repCapName, single_repCapName))
                        break;
                    */
...
ViStatus Rs_ClearBeforeRead (ViSession instrSession)
{
    ...
    checkErr (viReadSTB (instrSession, (ViPUInt16) &response));
